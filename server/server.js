...
app.get("/api/menu/:id", (req, res) => {
  ...
  const menuIt = menuData.find((item) => item.id === parseInt(req.params.id));
  res.send(menuIt);
});
...
app.get("/api/menu/stock/list", (req, res) => {
  // console.log(getAvail())
  res.send(getAvail());
});

app.get("/api/menu/stock/list/:id", (req, res) => {
  const iceCream = getAvail().find(
    (cream) => cream.id === parseInt(req.params.id, 10)
  );
  res.send(iceCream);
});
...
app.post("/api/menu", (req, res) => {
  ...
  menuData.push(newiceCreams);
  res.send(newiceCreams);
